...
| Windows | `choco install nodejs-lts` 또는 <https://nodejs.org/> 의 LTS .msi |
| Linux | `curl -fsSL https://deb.nodesource.com/setup_20.x \| sudo -E bash -` 후 `sudo apt install nodejs` |
...
backend 가 살아있는지 먼저 확인:

```bash
curl -i http://localhost:8000/        # macOS / Linux
```

```powershell
Invoke-WebRequest http://localhost:8000/   # Windows
```
